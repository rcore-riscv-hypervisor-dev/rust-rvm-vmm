...
impl RVM{
    ...
    pub fn add_memory_region(&self, gpa: u64, len: usize)->Result<MemoryRegion>{
        ...
        let ret = sys_ioctl(self.fd, RVM_GUEST_ADD_MEMORY_REGION, &args as *const _ as usize);
        if ret<0{
            return Err(AddMemoryRegionError(ret));
        }else{
            return Ok(MemoryRegion{
                gpa,
                data: unsafe {core::slice::from_raw_parts_mut(args.userspace_addr as *mut _, len)}
            });
        }
    }
